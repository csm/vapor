...
(def gopath "/usr/local/go/bin/go")

(defn module
  "Find, compile, and return the path of a module."
  [& {:keys [os arch name namespace] :or {namespace default-namespace}}]
  (shell/with-sh-env {"GOOS" os "GOARCH" arch "GOPATH" (get-work-dir)
                      "PATH" (str "/usr/local/go/bin:" (System/getenv "PATH"))}
    (shell/sh gopath "get" "-u" "github.com/csm/go-edn") ;; dumbfuck go boilerplate; TODO must be a workaround
